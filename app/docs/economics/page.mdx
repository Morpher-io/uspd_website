...
const stabilizerNFTContract = new ethers.Contract(STABILIZER_NFT_ADDRESS, stabilizerNFTAbi, provider);
const positionEscrowAddress = await stabilizerNFTContract.positionEscrows(tokenId);

// Create contract instance for the PositionEscrow
const positionEscrow = new ethers.Contract(positionEscrowAddress, positionEscrowAbi, provider);
```
...
const priceQuery = {
    price: ethers.parseUnits("3000", 18), // $3000 with 18 decimals
    decimals: 18,
    dataTimestamp: Math.floor(Date.now() / 1000),
    assetPair: ethers.keccak256(ethers.toUtf8Bytes("ETH/USD")),
    signature: "0x..." // Signed price attestation
};

// Get the collateralization ratio
const ratio = await positionEscrow.getCollateralizationRatio(priceQuery);
...
const backedShares = await positionEscrow.backedPoolShares();

// Get current stETH balance (collateral)
const stEthBalance = await positionEscrow.getCurrentStEthBalance();

console.log(`Position has ${ethers.formatEther(backedShares)} backed shares`);
console.log(`Position has ${ethers.formatEther(stEthBalance)} stETH collateral`);
...
positionEscrow.on("CollateralAdded", (amount, event) => {
    console.log(`Collateral added to position ${tokenId}: ${ethers.formatEther(amount)} stETH`);
    // Recalculate position health
});

// Monitor CollateralRemoved events
positionEscrow.on("CollateralRemoved", (recipient, amount, event) => {
    console.log(`Collateral removed from position ${tokenId}: ${ethers.formatEther(amount)} stETH`);
    // Recalculate position health
});

// Monitor AllocationModified events (changes in backed shares/liability)
positionEscrow.on("AllocationModified", (sharesDelta, newTotalShares, event) => {
    console.log(`Position ${tokenId} allocation changed by ${ethers.formatEther(sharesDelta)} shares`);
    console.log(`New total backed shares: ${ethers.formatEther(newTotalShares)}`);
    // Recalculate position health
});

// Monitor StabilizerNFT events for new positions
stabilizerNFTContract.on("StabilizerPositionCreated", (tokenId, owner, event) => {
...
### 6. Complete Liquidation Bot Example

Here's a simplified liquidation bot structure:

```javascript
class LiquidationBot {
    constructor(provider, stabilizerNFTAddress, liquidatorTokenId) {
        this.provider = provider;
        this.stabilizerNFT = new ethers.Contract(stabilizerNFTAddress, stabilizerNFTAbi, provider);
        this.liquidatorTokenId = liquidatorTokenId;
        this.monitoredPositions = new Map();
    }

    async scanAllPositions() {
        const totalSupply = await this.stabilizerNFT.totalSupply();
        
        for (let i = 1; i <= totalSupply; i++) {
            try {
                const owner = await this.stabilizerNFT.ownerOf(i);
                const escrowAddress = await this.stabilizerNFT.positionEscrows(i);
                
                this.monitoredPositions.set(i, {
                    owner,
                    escrowAddress,
                    escrow: new ethers.Contract(escrowAddress, positionEscrowAbi, this.provider)
                });
            } catch (error) {
    ...
    async checkPositionHealth(tokenId, ethUsdPrice) {
        const position = this.monitoredPositions.get(tokenId);
        if (!position) return null;

        const priceQuery = this.buildPriceQuery(ethUsdPrice);
        const ratio = await position.escrow.getCollateralizationRatio(priceQuery);
        const ratioPercent = Number(ratio) / 100;
        
        const threshold = this.calculateLiquidationThreshold(this.liquidatorTokenId);
        
        return {
            tokenId,
            ratio: ratioPercent,
            threshold,
            isLiquidatable: ratioPercent < threshold,
            backedShares: await position.escrow.backedPoolShares()
        };
    }

    async liquidatePosition(tokenId, sharesToLiquidate, priceQuery) {
        // Execute liquidation transaction
        const tx = await this.stabilizerNFT.liquidatePosition(
            this.liquidatorTokenId,
            tokenId,
            sharesToLiquidate,
            priceQuery
        );
