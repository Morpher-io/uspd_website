...
class LiquidationBot {
    constructor(privateKey, stabilizerNFTAddress, liquidatorTokenId) {
        ...
        this.stabilizerNFT = getContract({
    ...
    async scanAllPositions() {
        const totalSupply = await this.stabilizerNFT.read.totalSupply();
        
        for (let i = 1n; i <= totalSupply; i++) {
            try {
                const owner = await this.stabilizerNFT.read.ownerOf([i]);
                const escrowAddress = await this.stabilizerNFT.read.positionEscrows([i]);
                ...
                this.monitoredPositions.set(Number(i), {
                    owner,
                    escrowAddress,
                    escrow
                });
    ...
    async checkPositionHealth(tokenId, ethUsdPrice) {
        ...
        const ratio = await position.escrow.read.getCollateralizationRatio([priceQuery]);
        ...
        return {
            ...
            backedShares: await position.escrow.read.backedPoolShares()
        };
    }

    async liquidatePosition(tokenId, sharesToLiquidate, priceQuery) {
        // Execute liquidation transaction
        const hash = await this.stabilizerNFT.write.liquidatePosition([
