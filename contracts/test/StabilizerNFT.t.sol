...
contract StabilizerNFTTest is Test {
    ...
    function setUp() public {
        ...
        stabilizerNFT.initialize(address(uspdToken));
    }
    ...
    function testAddUnallocatedFundsSuccess() public {
        ...
        (uint256 totalEth, uint256 unallocatedEth,,,,) = stabilizerNFT.positions(1);
        assertEq(totalEth, 1 ether, "Total ETH should match sent amount");
        assertEq(unallocatedEth, 1 ether, "Unallocated ETH should match sent amount");
        assertEq(stabilizerNFT.lowestUnallocatedId(), 1, "Should be lowest ID");
        assertEq(stabilizerNFT.highestUnallocatedId(), 1, "Should be highest ID");
    }
    ...
    function testAddMoreUnallocatedFunds() public {
        ...
        (uint256 totalEth, uint256 unallocatedEth,,,,) = stabilizerNFT.positions(1);
        assertEq(totalEth, 3 ether, "Total ETH should be sum of both additions");
        assertEq(unallocatedEth, 3 ether, "Unallocated ETH should be sum of both additions");
    }
