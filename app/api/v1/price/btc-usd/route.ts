...
export async function GET() {
    try {
        ...
        const priceFromRedis = await redis.hget('markets:CRYPTO_BTC', 'close');

        if (!priceFromRedis) {
            throw new Error('Failed to fetch price from Redis: key "markets:CRYPTO_BTC" with field "close" not found.');
        }
        ...
        const assetPairString = 'MORPHER:BTC_USD';
